...
class SERVO(object):
    ...
    def position_to_position(self, motion_request:dict):
        ...
        for name in motion_request:
            k = self.axis_map[name]
            if self.axis[k].motorStatus[0]:
                print(self.axis[k].realPulse, m['abs_puls'])
...
def startup():
    servo = SERVO('10.90.252.53', 1030, 'win', 0.3)
...
if __name__ == '__main__':
    ...
    for r in rcvs:
        ...
        while rcv:
            time.sleep(0.1)
            if hmi_thread.is_alive() and servo_thread.is_alive():
                pass
            else:
                exit_flag = True
                rcv = None
    ...
    # rcv= rcvs[0]
    # while 1:
    #     x = input('input X target position:\n')
    #     y = input('input Y target position:\n')
    #     try:
    #         rcv = dict(
    #             abs_pos = dict(
    #                 X = dict(
